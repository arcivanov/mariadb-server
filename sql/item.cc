                               ...
                               Field *to, bool no_conversions)
{
  ...
  *null_value= from->is_null();
  DBUG_RETURN(to->store_field_maybe_null(from, no_conversions));
}
...
bool Item_trigger_row::set_value(THD *thd, sp_rcontext *ctx, Item **it)
{
  ...
  if (!(value= thd->sp_fix_func_item(it)))
    return true;
...
bool Item_trigger_row::check_cols(uint n)
{
  if (n != cols())
  {
    my_error(ER_OPERAND_COLUMNS, MYF(0), n);
